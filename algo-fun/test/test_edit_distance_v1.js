...
var editDistance = require('../lib/edit_distance_v1');


describe("字符串编辑距离，v1 >", function () {
	...
	it('SNOWY <--> SUNNY，编辑距离为3', function () {
		...
		var d = editDistance(src, dst);
		should(d).be.eql(3);
		console.log(editDistance.ops);
		editDistance(dst, src);
		should(d).be.eql(3);
	});

	xit('Instagram: Ke ---> INSTAGRAM: KE', function () {
		this.timeout(15000);
		var src = 'Instagram: Ke',
			dst = 'INSTAGRAM: KE';
		editDistance.ops = 0;
		var d = editDistance(src, dst);
		should(d).be.eql(9);
		console.log('Instagram: Ke ---> INSTAGRAM: KE：', editDistance.ops);
	});
